    ...
    <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden bg-forest">
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center hero-stagger">
        ...
        <p className="text-xs sm:text-sm font-medium tracking-widest uppercase text-sage/80 mb-6">
          {config.certifications} · Sober since {config.soberSince.split(' ')[2] || '2016'}
        </p>
        ...
        <div className="flex items-center justify-center gap-3">
          <img
            ...
            alt={`${config.ownerName} - ${config.ownerTitle}`}
            className="w-12 h-12 rounded-full object-cover border-2 border-sage/30"
          />
          <div className="text-left">
            <p className="text-white text-sm font-medium">{config.ownerName}</p>
            <p className="text-white/50 text-xs">Sober since {config.soberSince.split(' ')[2] || '2016'} · {config.certifications.split('·')[0].trim()}</p>
          </div>
        </div>
      </div>
...
function TrustStrip() {
  ...
  const facts = [
    { icon: <Clock className="w-5 h-5" />, text: `${years} Years Sober` },
    { icon: <Award className="w-5 h-5" />, text: config.certifications.split('·')[1]?.trim() || 'Certified CBT' },
          ...
          <div className="flex flex-col md:flex-row items-center gap-10 md:gap-16">
            ...
            <div>
              <blockquote className="font-display text-xl sm:text-2xl text-leaf leading-relaxed italic mb-6">
                "On {config.soberSince}, I made a decision that changed my life. I chose sobriety. Today, {years} years later, I help others find the same freedom."
              </blockquote>
              <Link
                to="/my-story"
...
function Credentials() {
  ...
  const points = [
    { label: 'Sober Since', value: `${config.soberSince} — ${years}+ years of personal recovery` },
    { label: 'Certifications', value: config.certifications },
    ...
    { label: 'Availability', value: config.location },
  ]
  ...
  const faqs = [
    {
      ...
      a: `A sobriety coach is a trained recovery professional who provides personalised, one-on-one support to help you achieve and maintain sobriety. Unlike therapists who focus primarily on mental health diagnoses, a sobriety coach works alongside you in your daily life — helping you navigate triggers, build new habits, and create a sustainable recovery plan. At Releaf, I combine my own personal recovery with certified techniques to offer guidance that's both evidence-based and deeply empathetic.`,
    },
    {
          ...
          {latestPosts.map((post, i) => (
            <SectionReveal key={post.slug} delay={i * 100}>
              <Link to={`/blog/${post.slug}`} className="card-hover block bg-white rounded-2xl border border-forest/8 p-7 h-full group">
                <p className="text-xs text-sage font-medium mb-3">{post.date} · by {config.ownerName.split(' ')[0]}</p>
...
export default function HomePage() {
  ...
  return (
    <>
      <Helmet>
        <title>{`${config.ownerTitle} | Releaf — ${config.ownerName}`}</title>
        <meta
          name="description"
          content={`I've been where you are. ${config.certifications}. Personalised sobriety coaching, sober companion support, and rehab transition guidance. Free 30-min consultation.`}
        />
        <meta property="og:title" content={`${config.ownerTitle} | Releaf — ${config.ownerName}`} />
        <meta property="og:description" content={`Sobriety coaching from someone who has been exactly where you are. ${config.certifications}.`} />
